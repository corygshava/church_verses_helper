...
function getallbooks() {
	let bks = [...bibleBooks.OldTestament,...bibleBooks.NewTestament];
	let res = [];

	bks.forEach((el,id) => {
		let [name,chapters] = el.split("|");
		let verses = getverses(name);
		res.push({name,chapters,verses});
	});

	return res;
}

function getverses(book) {
	let res = undefined;
	let found = 0,found_id = 0,thekey = "";

	Object.keys(bibleBookVerses).forEach((el,id) => {
		if(el.toLowerCase() == book.toLowerCase()){
			found_id = id;
			thekey = el;
			found += 1;
		}
